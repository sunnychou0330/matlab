...
function result = KH(profile)
    ...
    for nr = 1:NR
        ...
        for z2 = 1:NK
            [K(z2), responseTime, providers]=fitness(X(:,z2), services, NT);
        end
        ...
        for j = 1:MI 
            ...
            [Kf(j), responseTime, providers] = fitness(Xf(:,j), services, NT);
            if 2<=j
                if Kf(j-1)<Kf(j)
                    Xf(:,j) = Xf(:,j-1);
                    Kf(j) = Kf(j-1);
            ...
            for i = 1:NK
                ...
                [offspring_fit, responseTime, providers] = fitness(offspring, services, NT);
                ...
                [K(i), responseTime, providers] = fitness(X(:,i), services, NT);
                if K(i)<Kib(i)
                    Kib(i)=K(i);
                    Xib(:,i)=X(:,i);
                end         
